...
@synchronize_on_fields(fields_name={'field_3'}, check_pipeline=False)
def field_3_is_positive(sample) -> bool:
    return sample.get_field_3() > 0


GeneratedSample = SampleGenerator(name='GeneratedSample').add_dataset_field(field_name='rgb_image', field_type=np.ndarray, save_function=slm.save_compressed_numpy_array, load_function=slm.load_compressed_numpy_array) \
