...
def test_pipeline_bgr_to_rgb():
    ...
    converted = DataPipeline(data=image_bgr, use_gpu=False).convert_bgr_to_rgb().run().get_data()
...
def test_pipeline_rgb_to_bgr():
    ...
    converted = DataPipeline(data=image_rgb, use_gpu=False).convert_rgb_to_bgr().run().get_data()
...
def test_pipeline_around():
    ...
    converted = DataPipeline(data=array, use_gpu=False).around(2).run().get_data()
...
def test_pipeline_add_operation():
    ...
    def f(data):
        data[data > 10] = 10
        return data
    converted = DataPipeline(data=np.array([11, 9, 11, 7, 5, 15]), use_gpu=False).add_operation(f).run().get_data()
