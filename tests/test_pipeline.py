...
def test_pipeline_bgr_to_rgb():
    ...
    converted = DataPipeline(data=image_bgr, use_gpu=False).convert_bgr_to_rgb().run().obtain_data()
...
def test_pipeline_add_operation():
    ...
    def f(data):
        data[data > 10] = 10
        return data
    converted = DataPipeline(data=np.array([11, 9, 11, 7, 5, 15]), use_gpu=False).add_operation(f).run().obtain_data()
